...
- payload / handle 分离：具体 handle 绑定的裸名选择对象 payload，`&`、`*`、`^`、`~` 等帽子选择获准的 handle 视图；抽象泛型 `T` 绑定则保留完整的 `T` 值

帽子语法只是这套设计的结果之一，不是设计目的本身。它存在，是因为 Toka 需要一种紧凑且一致的方式区分 payload 操作和 handle 操作。

**设计谱系说明。** Toka 的帽子语法系独立设计；Cforall、Alusus 等系统已具有
相关机制。Toka 不主张这些字符、引用的默认内容访问或显式 handle 选择分别由
本项目首创；设计重点是把具体 handle 类别的视图选择与所有权、借用、重绑定及
资源契约整合。详见[设计历史与相关工作](docs/design_lineage.md)。
