...
report is deliberately not a publication gate: Redis release review requires
the current Linux CI `redis_real_service` evidence to be `pass`; PostgreSQL
owns its completed real-service gate in `tokalang/postgres`.
