...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='toka-negative-purposes-') as temp:
        ...
        for row in rows:
            ...
            assert normal.returncode == 1, (source, normal.returncode, normal.stderr)
            stderr = re.sub(r'\x1b\[[0-9;]*m', '', normal.stderr)
            ...
            for suffix, flags in (('o', ['-c']), ('ll', ['--emit-ir'])):
                artifact = work / (source.stem + '.' + suffix)
                assert not artifact.exists(), artifact
                rejected = compile(source, [*flags, '-o', str(artifact)])
                assert rejected.returncode == 1 and not artifact.exists(), (source, suffix, rejected.stderr)
            results.append(dict(case=row['case'], status='target-verified', parity=True, artifacts=False))
        ...
        print(f'{repaired} repaired purposes; {len(positives)} runtime controls; {2 * repaired} no-artifact checks; {31 - repaired} blocked cases NOT counted')
