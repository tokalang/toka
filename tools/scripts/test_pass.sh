...
echo ""
echo "Running Semantic Manifest build-profile test..."
if ! run_without_test_cache bash tools/scripts/test_semantic_manifest_build_profile.sh; then
    echo -e "${RED}Semantic Manifest build-profile test failed!${NC}"
    exit 1
fi
