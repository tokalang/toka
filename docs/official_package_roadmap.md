...
| `official/postgres` | qualified bounded PostgreSQL v3 wire codec plus secure TLS/SCRAM startup client; not yet a query client | deterministic protocol/SCRAM/TLS mock, plus locked/offline local-consumer import replay pass | serial simple-query client, then real-service compatibility evidence |
...
4. **`official/postgres`** — asynchronous PostgreSQL wire client. Its bounded
   protocol codec and secure startup client are now qualified: verified TLS
   and SCRAM are defaults, while plaintext and unverified TLS are explicit
   local/test policies. The next slice is one serial simple-query API with
   recovery and cancellation rules before broader client surface is added.
