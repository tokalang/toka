...
| `official/redis` | released [`0.2.0`](https://github.com/tokalang/redis/releases/tag/v0.2.0) standalone bounded RESP2 client | tagged GitHub Release archive, immutable catalog digest, standalone deterministic Linux/macOS qualification, Redis 7.4.x/8.2.x TCP/TLS service matrix, and fresh public-registry/offline consumer replay | maintain the package in [`tokalang/redis`](https://github.com/tokalang/redis); expand compatibility only through a separately scoped release |
