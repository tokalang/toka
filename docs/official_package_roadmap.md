...
| `official/openai_compat` | active v1 expansion from SSE adapter to transport-neutral Chat Completions protocol package | existing bounded streaming decoder; approved request/response/stream/assembler contract | complete the [`openai_compat v1`](official_openai_compat_v1.md) matrix and agent-service integration before standalone extraction |
