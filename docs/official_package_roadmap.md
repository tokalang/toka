...
| `official/openai_compat` | active v1 expansion from SSE adapter to transport-neutral Chat Completions protocol package | modular request JSON and bounded streaming decoder qualify; agent-service constructs requests through the package | complete the [`openai_compat v1`](official_openai_compat_v1.md) non-stream response and assembler matrix before standalone extraction |
