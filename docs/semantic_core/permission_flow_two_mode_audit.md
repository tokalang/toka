...
iteration. At that historical revision, direct nullable payload guard and
fresh-rebind lowering was also recorded. That surface is now superseded by
E0484-E0487 removal; only raw `nul *T` presence remains. The full
...
2. `CedeExpr` now classifies member/index/spread paths as Shared, so they do
   not rebuild payload authority. Safe nullable transfer is no longer an
   admitted surface; raw may-zero to non-zero transfer requires the existing
   same-path raw guard/unwrap proof. Direct named fields of eligible local records and
...
5. At the audit revision, source-less `.tki` replay covered imported
   reference-field destructuring, match/guard flow propagation, guarded unique
   transfer, unguarded nullable rejection, and an exported `$` field ceiling.
   The Safe nullable cases are now superseded by E0484-E0487; current presence
   replay is raw may-zero only. A general freeze/sealed-object ceiling remains
   outside the represented language model.
