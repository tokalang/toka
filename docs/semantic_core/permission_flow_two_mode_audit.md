...
| Safe nullable payload and owning-handle surface | `T?`, `none`, `nul ^T`, and `nul ~T` are in their removal-warning phase (W0409/W0411/W0410). Their dedicated guard/cede conformance matrix has been retired rather than migrated to another absence mechanism. Legacy lowering remains temporarily available only to support the warning transition; raw pointer/FFI null is outside this slice. | **Retiring**: no new semantic expansion is permitted. |
