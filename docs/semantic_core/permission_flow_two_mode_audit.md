...
**Audit target:** `359d315f` (`fix(sema): enforce declaration-backed permissions`)

**Audit date:** 2026-07-26

**Result:** the static authority layer is implemented and evidenced; the
two-mode flow RFC is **not yet implemented**. It must not be represented as a
closed 1.0 guarantee.

## Evidence basis

- `python3 tools/run_conformance.py`: 32 passed, 0 failed on 2026-07-26;
...
|---|---|---|
| Iron rule: a shared view never amplifies Payload authority | The static checker rejects direct use-site escalation for established bindings, but no common flow result is propagated through fresh aliases, returns, and patterns. | **Partial**: this is the governing target invariant, not yet a universal implementation guarantee. |
...
| Shared sources cannot gain payload authority | Pointer compatibility permits writable-to-readonly decay; safe raw payload access is gated by `unsafe`. | **Partial**: `~`, `&`, `*`, and ordinary aliases do not flow through one shared-capability ceiling, so this is not a universal guarantee. |
...
1. `CedeExpr` in `src/Sema/Sema_Expr.cpp` treats `cede` as a generic wrapper:
   it invalidates/marks a source but does not classify `^` versus `~`/`&`/`*`,
   derive a flow ceiling, require whole binding, or prove nullable refinement.
2. Local initialization in `src/Sema/Sema_Stmt.cpp` infers moves for unique
   RHS variables, but does not record how the fresh LHS derived P or preserve a
   referent immutability ceiling.
...
1. Add an internal `PermissionFlow` classification/result for every fresh
   initializer, cede argument, return, and pattern binder. It should contain
   mode (`Independent`, `Shared`, `UnsafeRaw`), incoming payload ceiling,
   null-state, and source path.
