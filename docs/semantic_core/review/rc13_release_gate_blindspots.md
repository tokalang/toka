...
A later full local CTest attempt was interrupted by the host sleeping overnight: 106/123 passed, one call-transfer receipt test failed because a shared Stage 0 fixture had been migrated to Stage 1 spelling, and sixteen tests exceeded their wall-clock timeouts (some after printing `PASS`). The shared fixture was restored and a separate explicit-`cede` positive was added. The call-transfer test then passed; **all 17 previously failed/timed-out tests passed in a serial `ctest --rerun-failed` (17/17, 791.83 s)**. These two runs do not constitute one clean 123/123 CTest result. Logs: `/Users/zhyi/GitDP/tokalang/validation/rc13-ctest-wip.log` and `/Users/zhyi/GitDP/tokalang/validation/rc13-ctest-failed-rerun.log`.
