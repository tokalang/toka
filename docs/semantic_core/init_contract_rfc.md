...
TKI, and source-less caller replay agreement for all of the following.
Signature contracts are declaration-recomputed on import; in particular, an
imported `init` formal makes the caller's exact place `Live` after the normal
call boundary. Callee fulfilment is checked while compiling the provider source
that emits its object. The `init_002_parameter` replay case then hides that
source and proves that the resulting `.tki + object` pair preserves the
caller-side contract and runtime ABI.

That parity is not a proof that an arbitrary detached object fulfils a
body-derived postcondition. A retained body may be rechecked only when it is
lowered and linked as the object generated by that same trusted compile action;
it cannot justify a separately supplied provider object. The stronger Level-B
case—accepting an independently distributed body-derived attestation—remains
closed until the semantic-manifest layer supplies accepted provenance and an
