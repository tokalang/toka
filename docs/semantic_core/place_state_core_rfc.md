...
The first P0.4b CFG slices move ordinary `if`/`else`, `guard`, and conditional
`loop` capture, restore, reachable-branch selection, and join to the same
`ExactPlaceFacts` value. They preserve the PAL and compatibility transactions
alongside that value; break/continue use the central `AnalysisState` path. The
remaining `for`, match, rollback, and outcome-arm paths remain explicitly
unqualified.
