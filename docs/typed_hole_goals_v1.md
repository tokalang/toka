...
`toka hole-goals --json path/to/source.tk` and
`tokac --hole-goals=json path/to/source.tk` emit deterministic requirements
for each parsed `hole`. The schema is
[`schemas/toka.hole-goals.v1.schema.json`](../schemas/toka.hole-goals.v1.schema.json).
...
- `underconstrained`: accepting the hole would require inference or candidate
  selection; and
- `unsupported`: the surrounding form would make the hole a place, transfer,
...
unsupported. A `null` contract never means an unconstrained hole is accepted.
...
fact to synthesize a candidate, then must replace `hole` and run a normal
...
The ABI gate is `tools/scripts/test_typed_hole_goals.py`. It freezes envelope
