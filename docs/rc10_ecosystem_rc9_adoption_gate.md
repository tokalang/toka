...
| `postgres` | `0.1.0` | Fail: removed `get_ref`, raw-null contract, and cede-forwarding mismatch | Source migration passes macOS at `7256b2ae`; Linux pool cleanup double-frees |
...
### `ECO-RC9-C02`: async pool cleanup in Redis and PostgreSQL
...
accepted as a test-spelling issue.

PostgreSQL initially passed on macOS and returned status 70 under Linux x64
emulation because its test observed the waiter for only 200 ms. Commit
`7256b2ae` extends that observation-only bound to two seconds; Linux then
reliably reaches the cancellation cleanup and reports `double free detected in
tcache 2`, while macOS still passes. The shared shape is now async pool waiter
cancellation/cleanup rather than a Redis-specific lease implementation.
...
2. Re-run OpenAI Compat, Redis, and PostgreSQL on Linux x64 after their runtime
   blockers close; six candidates have complete applicable local platform gates.
3. Rebuild the six candidate archives from their final tagged commits and
...
7. Keep all six qualified package CI definitions pinned to exact RC9 archive
