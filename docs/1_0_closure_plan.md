...
| `FZ-3-C01` | `FZ-3` | `Complete` | Raw may-zero pointer unwrap reached a temporary LLVM trap | Lowered through the stable runtime panic path and locked by an expected-panic fixture |
...
| `FZ-3-R02` | `FZ-3` | `Complete` | Same-name generic shape templates used legacy process-global instance and impl keys | Resolved-template identity now isolates layouts and generic methods; source/source-less replay, nominal mismatch, and cache regeneration evidence are recorded in `semantic_core/generic_shape_identity_closure.md` |
