...
| **`std/slab.tk`** | `Slab<T>`, generational `SlabID`, `insert`, `get`, `get_mut`, `remove`, `clear` | **Tier 2: Core/Std 1.0** | A `SlabID` is an index plus generation, never a raw address. `remove` and `clear` invalidate every affected prior ID; later slot reuse cannot revive a stale ID. |
