...
4. **Tier 4: Qualified Official Packages**: Independently versioned packages
   with executable package qualification in their canonical repositories and
   exact-version public consumer evidence. `official/redis` and
...
| **`official/unicode`** | [`v0.1.1`](https://github.com/tokalang/unicode/releases/tag/v0.1.1) Unicode 17.0.0 UAX #29 extended-grapheme segmentation | **Tier 4: Qualified Official** | Canonical source is [`tokalang/unicode`](https://github.com/tokalang/unicode), with an exact-version fixture in [`toka-examples`](https://github.com/tokalang/toka-examples/tree/main/registry_unicode_consumer). Segmentation is pure Toka over hash-locked data; malformed UTF-8 returns `UnicodeError`. Normalization, shaping, bidi, and host ICU integration remain outside this profile. |
| **`official/gui`** | [`v0.1.0`](https://github.com/tokalang/gui/releases/tag/v0.1.0) macOS window/Metal lifecycle slice and pure grapheme-selection adapter | **Tier 4: Qualified Official** | Canonical source is [`tokalang/gui`](https://github.com/tokalang/gui). Its two exact-release demos and [public consumer](https://github.com/tokalang/toka-examples/tree/main/registry_gui_consumer) qualify fresh registry resolution, strict two-archive offline rebuild, native Objective-C compilation, and AppKit/Metal/QuartzCore linkage. Hosted CI does not open a window, prove a Metal device, qualify interactive input/IME, or satisfy the cross-platform GUI 1.0 gate. |
| **`official/redis`** | [`v0.2.0`](https://github.com/tokalang/redis/releases/tag/v0.2.0) RESP2 client, ordered pipelines, `RedisPool` / exclusive `RedisLease` | **Tier 4: Qualified Official** | Canonical source is [`tokalang/redis`](https://github.com/tokalang/redis), with an exact-version fixture in [`toka-examples`](https://github.com/tokalang/toka-examples/tree/main/registry_redis_consumer). Dedicated package pool, not `Pool<T>`; cancellation, protocol, and reply-alignment failures poison a client before lease return. Real service rows: Redis 7.4.x/8.2.x TCP `AUTH` and private-CA TLS. |
